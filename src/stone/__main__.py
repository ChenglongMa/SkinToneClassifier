...
def main():
    ...
    if os.path.exists(result_filename):
        renamed_file = os.path.join(
            output_dir, now.strftime("./result_bak_%y%m%d%H%M.csv")
        )
        shutil.move(result_filename, renamed_file)
    header = (
        "file,image type,face id,"
        + ",".join(
            [f"dominant {i + 1},props {i + 1}" for i in range(n_dominant_colors)]
        )
        + ",skin tone,PERLA,accuracy(0-100)"
    )
            ...
            for result in pool.imap(partial_process_image, filenames):
                ...
                if debug:
                    debug_dir = os.path.join(
                        output_dir, f"./debug/{image_type}/faces_{n_faces}"
                    )
                    os.makedirs(debug_dir, exist_ok=True)
                    for face_id, report_image in report_images.items():
                        image_name = f"{basename}-{face_id}"
                        report_filename = os.path.join(
                            debug_dir, f"{image_name}{extension}"
                        )
                        with lock:
                            cv2.imwrite(report_filename, report_image)
                        # if is_single_file:
                        #     cv2.imshow(
                        #         f"Skin Tone Classifier - {image_name}", report_image
                        #     )
                        #     cv2.waitKey(0)
